...
- Added Custom Registry for `FFRecipe`, `LaboRecipe`, and `MillRecipe`
    - You can register your custom recipe by `RegistryEvent.Register<T>`
- Added new integration for `Mekanism`
    - `Gas Transfer`: transfers gas wirelessly
- **Rebuilt Core Classes**
- Removed old material registries and builders

### Blocks

- Fixed the bug that Tile Entity didn't read a NBT tag from ItemStack
- Fixed the behavior about Transfers

## v0.7.0a

- **Replaced IInventory with IItemHandler so that some existing tile entity will lose their data**
- Fixed the incorrect texture mapping when the block model rotated
- Removed integration for CraftTweaker
- [WIP] Added json config for registering custom materials
- [WIP] Added new integration for Waila / Hwyla for Heat capability
- [WIP] Added new JEI tab about information for Materials
- [WIP] Registered new capability HEAT

### Gradle Update

- ForgeGradle: 2.3.4 -> 5.1.+
- Gradle: 4.10.3 -> 7.6.1
- Kotlin: 1.3.0 -> 1.8.10
- Mappings: snapshot_20171003 -> stable_39

### Blocks

- FullBottle Station: Added Gui
- I.C.R.C, Laboratory Table: Added catalyst slot
- Ore Dictionary Converter: Redesigned with Tile Entity to automate conversion
- [New] Energy / Fluid Transfer: transfers energy / fluid wirelessly

### Items

- [WIP] Ender Tablet: open ender chest without interacting Ender Chest

### Materials

- **Registered missing materials from Strontium to Plutonium**
- Fixed incorrect index
    - EMERALD: 31 -> 41
    - AQUAMARINE: 32 -> 42
    - BORON_OXIDE: 51 -> 52
- Fixed incorrect translated name: iodine -> Iodine
