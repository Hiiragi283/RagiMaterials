    ...
    fun init() {
        for (material in RagiMaterial.list) {
            ...
            if (EnumMaterialType.INGOT_HOT in material.type.list) {
                ...
                CraftingManager.addShaped(Reference.MOD_ID + ":hot_ingot_to_gear_" + material.index, MaterialUtil.getPart(PartRegistry.GEAR, material), " A ", "ABA", " A ", 'A', "ingotHot${material.getOreDict()}", 'B', ItemStack(RagiItem.ItemForgeHammer, 1, OreDictionary.WILDCARD_VALUE))
                //hot ingot -> plateのレシピを登録
                CraftingManager.addShaped(Reference.MOD_ID + ":hot_ingot_to_plate_" + material.index, MaterialUtil.getPart(PartRegistry.PLATE, material), "AB", 'A', "ingotHot${material.getOreDict()}", 'B', ItemStack(RagiItem.ItemForgeHammer, 1, OreDictionary.WILDCARD_VALUE))
                //hot ingot -> stickのレシピを登録
                CraftingManager.addShaped(Reference.MOD_ID + ":hot_ingot_to_stick_" + material.index, MaterialUtil.getPart(PartRegistry.STICK, material, 4), "AB", "A ", 'A', "ingotHot${material.getOreDict()}", 'B', ItemStack(RagiItem.ItemForgeHammer, 1, OreDictionary.WILDCARD_VALUE))
            }
        }
    }
}