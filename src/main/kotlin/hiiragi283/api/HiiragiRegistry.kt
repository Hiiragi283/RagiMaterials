...
object HiiragiRegistry {
    ...
    fun initMaterial() {
        ...
        CACHE.clear()
        //indexを昇順に並べて代入する
        INDEX_MAP.putAll(materialInternal.map { it.value.index to it.value }.toMap())
    }
    ...
    fun initPart() {
        createAllParts().forEach { registerTag(it.getOreDicts(), it) }
    }

    @JvmStatic
    fun createAllParts(): List<HiiragiPart> =
        getShapes().flatMap { shape -> getMaterials().map { HiiragiPart(shape, it) } }

    @JvmStatic
    fun getPart(oredict: String): HiiragiPart = partInternal.getOrDefault(oredict, HiiragiPart.EMPTY)
    ...
    @JvmStatic
    fun getHeat(metaLocation: MetaResourceLocation): Int = heatInternal.getOrDefault(metaLocation, 273)
