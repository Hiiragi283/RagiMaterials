...
object PeriodSecond {
    ...
    fun load() {
        ...
        MaterialRegistry.LAPIS = RagiMaterial.Builder(101, "lapis", TypeRegistry.CRYSTAL.enableOre()).setComponents(listOf(RagiMaterial.Formula("?").build() to 1)).setMixture().apply {
            color = RagiColor.BLUE
            crystalType = EnumCrystalType.LAPIS
        }.build()

        MaterialRegistry.REDSTONE = RagiMaterial.Builder(102, "redstone", TypeRegistry.DUST.enableOre()).setSimple(ElementRegistry.REDSTONE to 1).build()

        MaterialRegistry.GLOWSTONE = RagiMaterial.Builder(103, "glowstone", TypeRegistry.DUST.enableOre()).setSimple(ElementRegistry.GLOWSTONE to 1).build()
